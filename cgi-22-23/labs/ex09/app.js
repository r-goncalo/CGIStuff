...
function setup(shaders){
    ...
    gl.bindBuffer(gl.ARRAY_BUFFER, bBuffer); //the aBuffer is binded
    gl.bufferData(gl.ARRAY_BUFFER, flatten(colors), gl.STATIC_DRAW); // we write the colors to the buffer
...
function animate(miliseconds){
    ...
    gl.uniform1f(uDx, Math.cos(dStep * miliseconds) * radius);
    gl.uniform1f(uDy, Math.sin(dStep * miliseconds) * radius);
